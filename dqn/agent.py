...
class Agent(metaclass=ABCMeta):
    ...
    def transitions_to_tensor(self, transitions):
        ...
        rews_t = (
            T.as_tensor(np.asarray([t[2] for t in transitions]), dtype=T.float32)
            .to(self.device)
            .unsqueeze(-1)
        )
        dones_t = (
            T.as_tensor(np.asarray([t[3] for t in transitions]), dtype=T.float32)
            .to(self.device)
            .unsqueeze(-1)
        )
    ...
    def store_transitions(self, obses, actions, rews, dones, new_obses, infos):
        for i in self.replay_memory_buffer.store_transitions(
        ...
        if self.epsilon_exp_decay:
            return np.exp(
                np.interp(
                    ...
                    [np.log(self.epsilon_start), np.log(self.epsilon_min)],
                )
            )
        else:
            return np.interp(
    ...
    def update_target_network(self, force=False):
        if (
            not self.target_soft_update
            and self.step % (self.update_target_frequency // self.n_env) == 0
        ) or force:
    ...
    def load_model(self):
        if self.load and os.path.exists(self.save_path):
            ...
            [
                ...
                for _ in range(np.min([self.episode_count, self.ep_info_buffer.maxlen]))
            ]
            print(
                "Step: ",
    ...
    def log(self):
        if self.step % self.log_frequency == 0 and self.step > self.resume_step:
            ...
            print(
                "Step: ",
                self.step * self.n_env,
                " (" + str(self.step) + "x" + str(self.n_env) + ")",
            )
            ...
            print(
                Fore.LIGHTRED_EX,
                "---",
                str(timedelta(seconds=round((time.time() - self.start_time), 0))),
                "---",
                Fore.RESET,
            )
            ...
            self.summary_writer.add_scalar(
                "Episodes", self.episode_count, global_step=(self.step * self.n_env)
            )
...
class DQNAgent(SimpleAgent):
    def __init__(self, *args, **kwargs):
        ...
        self.online_network = DeepQNetwork(
            self.device, self.lr, self.nn_conf_func, self.input_dim, self.output_dim
        )
...
class PerDuelingDoubleDQNAgent(PerDoubleAgent):
    def __init__(self, *args, **kwargs):
        ...
        self.replay_memory_buffer = ReplayMemoryPrioritized(
            self.buffer_size, self.batch_size, self.epsilon_decay
        )
